...
<template>
    <div class="flex items-center justify-between px-2 py-4">
        ...
        <div class="flex items-center space-x-6 lg:space-x-8">
            ...
            <div class="flex items-center space-x-2">
                <Button variant="ghost" class="hidden w-8 h-8 p-0 lg:flex" :disabled="!table.getCanPreviousPage()"
                    @click="table.setPageIndex(0)">
                    <span class="sr-only">Go to first page</span>
                    <!-- <DoubleArrowLeftIcon class="w-4 h-4" /> -->
                    <ChevronDoubleLeftIcon class="w-4 h-4" />
                </Button>
                <Button variant="ghost" class="w-8 h-8 p-0" :disabled="!table.getCanPreviousPage()"
                    @click="table.previousPage()">
                    <span class="sr-only">Go to previous page</span>
                    <ChevronLeftIcon class="w-4 h-4" />
                </Button>
                <Button variant="ghost" class="w-8 h-8 p-0" :disabled="!table.getCanNextPage()"
                    @click="table.nextPage()">
                    <span class="sr-only">Go to next page</span>
                    <ChevronRightIcon class="w-4 h-4" />
                </Button>
                <Button variant="ghost" class="hidden w-8 h-8 p-0 lg:flex" :disabled="!table.getCanNextPage()"
