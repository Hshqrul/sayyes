...
<template>
    ...
    <FormSimpleLayout :title="event.event_name" :description="event.description">
        <div class="flex flex-col gap-4 mb-4">
            ...
            <div class="border-b border-sidebar-border/70"></div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 overflow-y-auto max-h-[calc(100vh-12rem)] sm:max-h-[calc(100vh-16rem)] md:max-h-[calc(100vh-20rem)] lg:max-h-[calc(100vh-24rem)]">
                <RsvpCard v-for="(rsvp, index) in rsvps" :key="index" :name="rsvp.name"
                    :body="rsvp.notes || `No message left`" :attendance="rsvp.attendance" :no_of_pax="rsvp.no_of_pax"
